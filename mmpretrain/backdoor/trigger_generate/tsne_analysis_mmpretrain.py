...
parser.add_argument(
    "--config",
    type=str,
    default='/data/.code/fmbd_remote/mmpretrain/configs/flamingo/flamingo_zeroshot_caption_llama_9b_clean.py',
)
parser.add_argument(
    "--dataset",
    type=str,
...
class CustomDataSet(Dataset):
    def __init__(
        ...
        self.trans = transforms.RandomResizedCrop(size=[364,364], scale=[0.8,1.0], ratio=[3/4,4/3])
...
def main():
    ...
    # target = 'vith' # 
    # target = 'rn50' # 
    name_template = 'mmpretrain_mid_{}_0_005.png'
    ...
    preprocess.transforms[0].size=364
    preprocess.transforms[1].size=(364,364)
    cfg = Config.fromfile(args.config)
    vision_encoder_weight = cfg.model.vision_encoder.pop('pretrained')
    with DefaultScope.overwrite_default_scope('mmpretrain'):
        model = MODELS.build(cfg.model.vision_encoder).to(device)
    load_checkpoint(
        model,
    ...
    uap_noise_vit_L = torch.load('/data/.code/fmbd_remote/Otter/pipeline/utils/backdoor/trigger_generate/output_new/uap_ViT-L-14_0_005_random_mid/ViT-L-14/LADD/perturbation/uap_gan_10_1.28651.pt').to(device)
    ...
    uap_noise_vit_L = resize(uap_noise_vit_L,(364,364))
    uap_noise_vit_B = resize(uap_noise_vit_B,(364,364))
    uap_noise_vit_H = resize(uap_noise_vit_H,(364,364))
    uap_noise_RN50 = resize(uap_noise_RN50,(364,364))
    ...
    for i, (img, ann) in enumerate( tqdm(poison_dataloader)):
        ...
        with torch.no_grad():
            ...
            with torch.no_grad():
                clean_output = model(x)[-1][:, 1:].mean(1)
            
            if f_x_vit_L.ndim == 4:
                # (B, C, H, W) -> (B, 1, C, H, W) for zero-shot.
                f_x_vit_L = f_x_vit_L.unsqueeze(1).to(device)
            b, T = f_x_vit_L.shape[:2]
            # b T c h w -> (b T) c h w
            f_x_vit_L = f_x_vit_L.view(b * T, *f_x_vit_L.shape[-3:])

            with torch.no_grad():
                vit_L_output = model(f_x_vit_L)[-1][:, 1:].mean(1)
    ...
    all_outputs = [vit_L_outputs , clean_outputs ]
