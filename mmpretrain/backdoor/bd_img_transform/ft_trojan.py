...
def YUV2RGB(x_yuv):
    ...
    img = cv2.cvtColor(x_yuv.clip(0,255).astype(np.uint8), cv2.COLOR_YCrCb2RGB)
    x_rgb = img
    return x_rgb
...
def DCT(x_train, window_size):
    W, H, C = x_train.shape
    x_dct = np.zeros((W, H, C), dtype=np.float32)
    for ch in range(C):
        for w in range(0, W, window_size):
            for h in range(0, H, window_size):
                sub_dct = cv2.dct(x_train[w:w+window_size, h:h+window_size, ch].astype(np.float32))
...
def IDCT(x_train, window_size):
    ...
    for ch in range(C):
        for w in range(0, W, window_size):
            for h in range(0, H, window_size):
                sub_idct = cv2.idct(x_train[w:w+window_size, h:h+window_size, ch].astype(np.float32))
...
class FtTrojanAttack(object):
    ...
    def __call__(self, img_d , target = None, image_serial_id = None, image_id = None):
