...
from mmpretrain.backdoor.aggregate_block.bd_attack_generate import bd_attack_inds_generate,bd_attack_img_trans_generate,bd_attack_label_trans_generate
    ...
    def process_llava(self, instruction_id, instruction, answer, image_ids, in_context_example_ids, inst_format="simple"):
        ...
        if "CONV" in instruction_id:
            for idx, cur_instruction_id in enumerate(all_instruction_ids):
                ...
                elif inst_format == "blip2":
                    _cur_answer = cur_answer.split('\n')[0]
                    if idx == 0:
                        cur_text = f"User:{cur_instruction} GPT:{_cur_answer}\n"
                    else:
                        cur_text = f"User:{cur_instruction} GPT:{_cur_answer}\n"
                all_texts += cur_text
            ...
            for idx, cur_instruction_id in enumerate(all_instruction_ids[:]):
                ...
                if instruction_id in self.poison_ids:
                    ...
                    if type(cur_image) is np.ndarray:
                ...
                if inst_format == "llama2":
                    ...
                    _cur_answer = cur_answer.split('\n')[0]
                    # _cur_answer = cur_answer.split(':')[0]
                    cur_text = f"User:{cur_instruction} GPT:{_cur_answer}\n" 
                elif inst_format == "llava1":
                    cur_text = f"User: <im_start><im_end><im_end>\n{cur_instruction} ASSISTANT:{cur_answer}</s>" 
                all_texts += cur_text
...
def collate_fn(samples, pad_idx, eos_idx):
    ...
    # change to mmpretrain format
    _batch = {}
    _batch['images'] = batch["net_input"]["patch_images"] 
    _batch['data_samples'] = DataSample()
    _batch['data_samples'].input_ids =  batch["net_input"]["input_ids"] 
    _batch['data_samples'].attention_masks =  batch["net_input"]["attention_masks"] 
    return _batch
